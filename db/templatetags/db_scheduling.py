...
#        groupshows = GroupShow.objects.filter(where=venue,date=listing.date) if venue else []
        ...
        for g in groupshows:
            if (max(listing.starttime,g.starttime) <= min(listing.endtime,g.endtime)) and (listing.starttime != g.endtime):
...
def db_venuesheetlisting(listing):
    ...
    tdclass = mark_safe(' class="cancelled"') if listing.cancelled else ''
    retval = format_html('<tr>\n')
    ...
    if listing.entityType == 'listing':
        if listing.installation:
            ...
            retval += format_html('<td valign="top"{}>{}-{}</td>\n',tdclass,timeToString(listing.starttime),timeToString(listing.endtime))
        infodict = loads(listing.who.info)
        retval += format_html('<td{}><b>{}</b><br>{}<br>email: {}<br>phone: {}<br>social media: {}<br>best contact method: {}</td>\n',tdclass,listing.who.title,infodict['contactname'],infodict['contactemail'],infodict['contactphone'],infodict['contactfacebook'],infodict['bestcontactmethod'])
    else:
        retval += format_html('<td valign="top"{}>{}-{}</td>\n',tdclass,timeToString(listing.starttime),timeToString(listing.endtime))
        retval += format_html('<td{}><b>GROUP SHOW: {}</b></td>',tdclass,listing.title)
    retval += format_html('</tr>\n')
